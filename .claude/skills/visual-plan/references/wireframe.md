...
<!-- PROJECT-HOUSE-STYLE:tender-ai START (此區塊在 SHARED-CORE 之外；`skills update` 只覆寫 shared core，不會動到這裡) -->

## 專案房規（Tender AI）— clean design 覆寫

> 本節是 Tender AI 專案的固定視覺標準，**覆寫上方 SHARED-CORE 的「hand-drawn font／rough.js sketch overlay／never set font-family／No decorative shadows」等預設**。在本專案產生任何 wireframe、`<Screen>`、`<PrototypeScreen>` 時一律以本節為準，不得回退手繪風。

**1. 一律 clean design、零抖動零手寫。** 每個 `<Screen>` / `<PrototypeScreen>` 都要設 `renderMode="design"`，關閉 rough.js 草圖疊層與手寫字。線條為純直線、可倒圓角，禁止抖動、手繪感、歪斜或假草稿質感；資訊層級用 Bento 卡片分區 ＋ 間距 ＋ 邊框 ＋ 背景建立。

**2. 字體（覆寫「never set font-family」）。** `renderMode="design"` 開放 `css={}` 覆寫字體，務必用 `[lang]`-scoped css 鎖定：繁體中文 `[lang="zh-Hant"]` **只能**用 `"Noto Sans TC", sans-serif`；英文 `[lang="en"]` 用 `"Inter", "SF Pro Text", sans-serif`；數字與程式碼（`.mono` / `code`）用 `"JetBrains Mono", "SFMono-Regular", monospace`。中文不得用 Noto Sans TC 以外字型，wireframe 內不得出現裝飾性／手寫體字型。

**3. 統一 16px 圓角 ＋ Bento 卡片。** `.wf-card` / `.wf-box` 一律 `border-radius:16px`、`border:1px solid var(--wf-line)`、`background:var(--wf-card)`，用卡片切出不同文字區塊；pill/chip 用 `border-radius:999px`，按鈕／輸入框用 10px。

**4. 些微陰影 OK（覆寫「No decorative shadows」）。** 允許在卡片加**極淡**陰影強化分區，例如 `box-shadow:0 1px 2px rgba(0,0,0,.06)`；僅限輕微深度，仍禁止濃重投影、發光、`filter: drop-shadow()` 或 Tailwind `shadow-lg` 等誇張效果。`.wf-box`、按鈕、輸入框維持平面。

**5. 繁中為預設 ＋ 單一語系切換鈕。** 預設語系 `zh-Hant`；Topbar 放單一 `EN／中` 切換鈕，只改顯示語系，不重設篩選、收藏、評價或 Dialog 狀態。新文案在 `strings.ts` 的 zh／en 成對新增，重用 `AppProvider` 的 `loadLang()`／`saveLang()`。

**6. 標準 `css={}` 範本（依語系換第 3 行字體選擇器）：**
...
顏色仍一律用 `--wf-*` token（勿寫死 hex），讓 light／dark 正常翻轉。

<!-- PROJECT-HOUSE-STYLE:tender-ai END -->
