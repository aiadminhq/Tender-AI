...
<!-- PROJECT-HOUSE-STYLE:tender-ai START — 本段在 SHARED-CORE 之外，skills update 不會覆蓋 -->

## Tender AI 畫布房規（house style）

本專案 canvas 沿用 `references/wireframe.md` 末段「Tender AI Wireframe 房規」的同一套規則：每個 `<Screen>` 一律 `renderMode="design"`（純直線、零抖動、零手寫、無預設手寫字與 rough overlay）；字體以 `[lang]` 範圍綁定（繁中只用 `Noto Sans TC`、英文用 `Inter`／`SF Pro Text`、數字與程式碼用 `JetBrains Mono`／`SFMono`）；統一 16px 圓角與 Bento 卡片分區；卡片可帶些微陰影（`box-shadow:0 1px 2px rgba(0,0,0,.06)`），但禁止濃重投影或假深度。`<Annotation>` 仍維持純文字層，不加邊框或卡片陰影。完整 `css={}` 範本見 wireframe.md 房規段。
