...
- 「抓 PCC 詳情建檔 + 灌向量」必須在**能連到 PCC／Ollama／DB 的本機**執行 —— 由 launchd 觸發 Claude Code headless 完成。
...
| `scripts/automation/run_claude_daily.sh`               | launchd 包裝器：載入環境後跑 `claude -p /tender-daily`，無 claude 時退回直接跑引擎 |
