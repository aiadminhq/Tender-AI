...
> 📍 **現況（2026-06-25）**：P4 學習迴圈已落地——關鍵字權重學習（TF 詞頻比較／極性推導）、預算軟閾值、append-only `KeywordWeightRevision` 審計、`self_evolve` 觸發閘（team 線 consent-aware 樣本 ≥50 且較上批有新增）皆已實作並有測試；另加**三分判斷即時學習**（`POST /tenders/{id}/evaluate`）。
>
> ⚠️ **作廢更正**：本文 §1.1 將「財物／勞務」建議為自動 `negative` 的對映表**已作廢、未採用**。現行鐵則為 **財物／勞務 預設中性 `0.0`，不自動給負分**；正向可自動學、**負分一律人工專屬**（系統只列候選＋附理由建議，由人確認）。唯一例外為使用者本人 UI 三分判斷的即時學習（owner 知情覆寫）。詳見 `CLAUDE.md` 與記憶 `negative-keywords-human-only`。閱讀以下分析時請以此更正為準。
...
```python
# ⚠️ 已作廢：財物／勞務 不採自動負分（見頂部更正）。實際只保留正向先驗：
# 在 learn_keywords.py 中加入顯式的分類對映表
CATEGORY_POLARITY = {
    "工程": ("positive", 1.0),      # 100% 可行率 → 採用（正向先驗，約 +0.18）
    "營繕工程": ("positive", 1.0),  # 100% 可行率 → 採用
    # "財物": ("negative", ...),    # ❌ 不採用：預設中性 0.0，負分人工專屬
    # "勞務": ("negative", ...),    # ❌ 不採用：預設中性 0.0，負分人工專屬
}
```
