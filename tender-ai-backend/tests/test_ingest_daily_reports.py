...
async def test_ingest_daily_reports_idempotent(db_session, tmp_path):
    ...
    # 驗證每日快照有落地：有潛力等級的標案應寫入 daily_tender（tier 非 NULL），
    # dashboard 高潛力 KPI 與分級篩選讀這裡；此前日報匯入漏寫導致 tier 恆 NULL。
    snapshot_rows = await db_session.execute(select(DailyTender))
    snapshots = snapshot_rows.scalars().all()
    assert len(snapshots) > 0
    assert all(s.tier in ("high", "mid") for s in snapshots)
